...
function ArticleSelector({ onSelect }: { onSelect: (a: Article) => void }) {
  return (
    <div className="min-h-screen bg-page text-primary flex flex-col items-center px-4 py-10 gap-6">
      <h1 className="text-2xl font-bold tracking-widest text-amber-400">文章练习</h1>
      <p className="text-sm text-muted">选择一篇文章开始跟打练习</p>
        ...
        {ARTICLES.map(article => {
          ...
          return (
            <button
              ...
              className="text-left p-4 rounded-2xl border border-theme bg-card hover:border-amber-400/40 hover:bg-amber-400/5 transition-all group"
            >
              ...
              <div className="text-xs text-muted mb-2">{article.description}</div>
              <div className="text-xs text-faint">{chars.length} 个可打字符</div>
            </button>
          )
        })}
      </div>
...
function ResultOverlay({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-page/90 backdrop-blur">
      <div className="w-80 rounded-2xl border border-amber-400/30 bg-card p-8 flex flex-col items-center gap-6">
        ...
        <div className="w-full grid grid-cols-2 gap-4">
          {[
            ...
            <div key={label} className="flex flex-col items-center p-3 rounded-xl bg-chip">
              <div className="text-2xl font-bold text-white tabular-nums">
                {value}<span className="text-sm text-muted ml-0.5">{unit}</span>
              </div>
        ...
        <div className="flex gap-3 w-full">
          ...
          <button
            onClick={onBack}
            className="flex-1 py-2 rounded-xl bg-chip text-secondary font-semibold text-sm hover:bg-chip-hover transition-colors"
          >
            换篇文章
          </button>
        </div>
...
function ArticlePractice({
  ...
  return (
    <div
      className="min-h-screen bg-page text-primary flex flex-col px-4 py-8 gap-4"
    ...
    >
      ...
      <div className="flex items-center justify-between max-w-2xl w-full mx-auto">
        ...
        <div className="flex items-center gap-4 text-sm">
          <span className="text-secondary">
            ...
            <span className="text-faint ml-1">字/分</span>
          </span>
          <span className="text-secondary">
          ...
          <button
            ...
            className="text-xs px-2 py-1 rounded bg-chip hover:bg-chip-hover text-secondary transition-colors"
          >
            {paused ? '▶ 继续' : '⏸ 暂停'}
          </button>
        </div>
      ...
      {/* 进度条 */}
      <div className="w-full max-w-2xl mx-auto h-1 bg-chip rounded-full overflow-hidden">
        <div
      ...
      <div
        className={`
          ...
          ${paused ? 'opacity-40 border-theme bg-card' : 'border-theme bg-card'}
        `}
      >
        {chars.map((c, i) => {
          ...
          if (c.isPunctuation || !c.code) {
            return (
              <span key={i} className="text-faint">{c.char}</span>
            )
          }
